---
title: Guide for AWS
description: This doc will help you setup NodeKeeper on AWS Account
---

NodeKeeper makes it easy to setup and maintain a standalone blockchain node on AWS. This guide will help you with the steps that you need to perform for NodeKeeper.
...
For NodeKeeper to be able to crete and manage resources on your AWS account, you need to create an access key.
...
<Tabs>
    <Tab title="AWS Console">
        ...
        1. Go to user's tab on IAM console, search for user `scale3-nodekeeper` and select it.
    ...
    <Tab title="AWS Cli">
        ...
        1.  Create AWS User
            ```bash
            aws iam create-user --user-name scale3-nodekeeper
            ```
        2. Attach required permissions to the user
            ```bash
            aws iam attach-user-policy --user-name scale3-nodekeeper --policy-arn arn:aws:iam::aws:policy/AmazonEC2FullAccess
            ```
        ...
        1. Create Access Key
            ```bash
            aws iam create-access-key --user-name scale3-nodekeeper
            ```
...
If you want to remove NodeKeeper access, you have to delete the access key, user or permissions that were created.
...
<Tabs>
    <Tab title="AWS Console">
        ...
        1. Login to your AWS account.
        2. Open IAM console https://console.aws.amazon.com/iamv2/home
        3. IAM → From left menu, select users → Search for `scale3-nodekeeper` user and select it.
    ...
    <Tab title="AWS Cli">
        ...
        1. Delete All Access Keys
            ```bash
            IAM_USER_NAME="scale3-nodekeeper"
        ...
        2. Detach Policies
            ...
            aws iam delete-policy --policy-arn arn:aws:iam::$SCALE3_AWS_ACCOUNT_ID:policy/scale3-nodekeeper-policy
            ```
            </Accordion>
        3. Delete User
            ```bash
            aws iam delete-user --user-name scale3-nodekeeper
            ```
    </Tab>
